// Zyvra server (Express) — Vercel-ready, ROOT-safe paths
...
const ADMIN_TOKEN = process.env.ADMIN_TOKEN || 'admin_dev';
const JWT_SECRET = process.env.JWT_SECRET || 'jwt_dev';
const JWT_DAYS = parseInt(process.env.JWT_DAYS || '365', 10);

// ---- Paths: use project ROOT so Vercel bundling doesn't break
const ROOT = process.cwd();
const DATA_DIR   = path.join(ROOT, 'data');
const PUBLIC_DIR = path.join(ROOT, 'Public'); // your folder is "Public"
...
function ensureFile(f, fb){ ensureDir(path.dirname(f)); if(!fs.existsSync(f)) fs.writeFileSync(f, JSON.stringify(fb,null,2)); }
function readJson(f, fb={}){ try{ return JSON.parse(fs.readFileSync(f,'utf8')); }catch(e){ return fb; } }
function writeJson(f, obj){ ensureDir(path.dirname(f)); fs.writeFileSync(f, JSON.stringify(obj,null,2)); }
...
// Local dev only; on Vercel static is served by vercel.json
...
function verifyPassword(pw, packed){
  const [salt,h]=String(packed||'').split('$'); if(!salt||!h) return false;
  const calc=crypto.pbkdf2Sync(pw,salt,100000,64,'sha512').toString('hex');
  try{ return crypto.timingSafeEqual(Buffer.from(calc,'hex'), Buffer.from(h,'hex')); }catch{ return false; }
}
function signToken(user){ return jwt.sign({ sub:user.id, email:user.email, name:user.name }, JWT_SECRET, { expiresIn:`${JWT_DAYS}d` }); }
...
function requireAuth(req,res,next){
  ...
  try{ req.user = jwt.verify(m[1], JWT_SECRET); next(); }catch{ return res.status(401).json({error:'bad_token'}); }
}
...
app.post('/api/admin/news', requireAdmin, (req,res)=>{
  ...
  db.articles.push(art); writeJson(NEWS_FILE,db);
...
app.delete('/api/admin/news/:id', requireAdmin, (req,res)=>{
  const id=req.params.id, db=readJson(NEWS_FILE,{articles:[]});
  const n=db.articles.length; db.articles = db.articles.filter(x=>x.id!==id);
  writeJson(NEWS_FILE,db);
...
app.get('/api/search',(req,res)=>{
  ...
  const items=db.articles.filter(a=>hit(a.title)||hit(a.description)||hit(a.content)||hit(a.source)||hit(a.category))
    .sort((a,b)=> new Date(b.publishedAt||0)-new Date(a.publishedAt||0));
  res.json({items});
});

// ================= TICKER =================
app.get('/api/ticker',(_req,res)=> res.json(readJson(TICKER_FILE,{text:"",url:"/",active:false,speedSec:16})));
app.post('/api/admin/ticker', requireAdmin, (req,res)=>{
  const b=req.body||{};
  const t={ text:String(b.text||'').trim(), url:String(b.url||'/'), active:!!b.active, speedSec:Math.max(6,parseInt(b.speedSec||16,10)) };
  writeJson(TICKER_FILE,t); res.json({ok:true,ticker:t});
});

// ================= AUTH =================
app.post('/api/auth/signup',(req,res)=>{
  const {name='',email='',password=''}=req.body||{};
  const nm=String(name).trim(), em=String(email).trim().toLowerCase(), pw=String(password);
  ...
  db.users.push(user); writeJson(USERS_FILE,db);
  const token=signToken(user);
  res.json({ok:true,token,user:{id:user.id,name:user.name,email:user.email}});
});
app.post('/api/auth/login',(req,res)=>{
  const {email='',password=''}=req.body||{};
  const em=String(email).trim().toLowerCase(), pw=String(password);
  const db=readJson(USERS_FILE,{users:[]}); const user=db.users.find(u=>u.email===em);
  if(!user || !verifyPassword(pw,user.pass)) return res.status(401).json({error:'bad_credentials'});
  const token=signToken(user); res.json({ok:true,token,user:{id:user.id,name:user.name,email:user.email}});
});
app.get('/api/auth/me', requireAuth, (req,res)=> res.json({ok:true,user:req.user}));

// ================= CRICKET (static empty) =================
app.get('/api/cricket/raw', (_req,res)=> res.json({ok:true,mode:'static-empty'}));
app.get('/api/cricket/diag',(_req,res)=> res.json({ok:true,mode:'static-empty'}));
app.get('/api/cricket/feed',(_req,res)=>{
  const now=new Date(); const pk=new Date(now.getTime()+5*60*60*1000).toISOString().slice(0,10);
  res.json({ live:[], results:[], upcoming:[], meta:{ tz:'Asia/Karachi', day:pk, cachedAt:now.toISOString() } });
});

// ================= HEALTH =================
app.get('/api/health', (_req,res)=> res.json({ ok:true, time:new Date().toISOString(), root:ROOT }));
...
${a.url?`<p style="margin-top:16px"><a href="${esc(a.url)}" target="_blank" class="neon-btn">Original source</a></p>`:''}
</main></body></html>`);
});
