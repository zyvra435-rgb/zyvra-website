...
async function loadNews(category='latest'){
  ...
  try{
    ...
    if(!r.ok){ grid.innerHTML = '<p style="color:#f88;text-align:center">Failed to load news.</p>'; return; }
    const js = await r.json();
    const items = Array.isArray(js.items) ? js.items : [];
    if(!items.length){ grid.innerHTML = '<p style="color:#8aa;text-align:center">No articles yet.</p>'; return; }
    grid.innerHTML = items.map(cardHTML).join('');
  }catch{ grid.innerHTML = '<p style="color:#f88;text-align:center">Failed to load news.</p>'; }
}
function cardHTML(a){
...
async function loadTicker(){
  try{
    ...
    if(t && t.active && t.text){
      ...
      link.style.position='relative'; requestAnimationFrame(step);
